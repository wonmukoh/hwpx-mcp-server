 ...
 * ## 왜 필요한가
 *
 * 표 전체에 테두리 한 가지만 걸면 **격자**가 된다.
 * 사례집·안내 자료의 표는 세로선을 걷어내고 가로선만 남기거나,
 * 한 칸만 배경을 깔거나, 위쪽에만 굵은 색선을 긋는다.
 * 그건 칸마다 `borderFill` 이 달라야 나온다 — 표 하나에 하나로는 못 만든다.
 *
 * ## 자리 적는 법
 *
 * `row` 만 주면 **그 줄 전체**, `col` 만 주면 **그 열 전체**,
 * 둘 다 주면 그 칸 하나, 둘 다 안 주면 표 전체다.
 * 목록에 적은 **차례대로 덮어쓴다** — 넓은 것을 먼저, 좁은 것을 나중에 적는다.
 *
 * `headers` 를 줬으면 **머리 줄이 0번**이다.
 *
 * ## 면 적는 법
 *
 * `"0.4 mm #2A5DA8"` 처럼 굵기와 색을 한 글자로 적는다.
 * 색을 빼면 검정, 굵기를 빼면 `0.12 mm`. 선을 지우려면 `"none"`.
 */
export interface 칸꾸밈 {
  /** 몇 번째 줄 (0부터). 빼면 **모든 줄** */
  row?: number;
  /** 몇 번째 칸 (0부터). 빼면 **모든 칸** */
  col?: number;
  /** 배경색 `#RRGGBB`. `none` 이면 안 채운다 */
  background?: string;
  /** 위 선. `"0.4 mm #2A5DA8"` · `"none"` */
  top?: string;
  /** 아래 선 */
  bottom?: string;
  /** 왼쪽 선 */
  ...
  /** 오른쪽 선 */
  right?: string;
  /** 이 칸만 가로 정렬 (`left` `center` `right`). `col_align` 을 덮는다 */
  align?: string;
...
export interface 표블록 {
   ...
   * 칸 안 줄 간격 (%).
   *
   * **안 주면 문서의 기본 문단모양을 따른다** — 그게 본문용이라 표에는 너무 넓다.
   * 실측: 빈 문서 틀의 기본값이 9pt 글자에 265% 로 그려져,
   * 한 쪽에 들어갈 표가 1.3쪽이 됐다. 촘촘한 표는 130~150 을 준다.
   */
  line_spacing?: number;
  ...
  border_width?: string;
  /**
   * 표 전체의 테두리 색 `#RRGGBB`. 기본 검정.
   *
   * 격자를 아예 안 그리려면 `border_width: 'none'` 을 준다 —
   * 그러고 나서 `cells` 로 **그을 선만** 긋는 것이 이 표의 쓰는 법이다.
   */
  border_color?: string;
  /**
   * 칸마다 다르게 줄 것. 격자를 걷어내고 가로선·배경으로 가르는 표는 이게 있어야 만들어진다.
   *
   * 적은 차례대로 덮어쓴다 — 넓은 것(줄·열)을 먼저, 좁은 것(칸 하나)을 나중에.
   */
  cells?: 칸꾸밈[];
  /**
   * 이 블록의 글꼴 이름 (예: `휴먼명조`).
 ...
 * 타원 15개(1편)·다각형 3개(1편)는 드물지만 **뼈대가 사각형과 같아** 같이 낸다.
 */
export interface 도형블록 {
   ...
   * 무엇을 그릴까. 안 주면 `rect`.
   *
   * 셋은 앞뒤가 다 같고 **가운데 기하만 다르다** (실측) —
   * 사각형은 네 꼭짓점, 타원은 중심과 두 축, 다각형은 꼭짓점 목록.
   */
  shape?: 'rect' | 'ellipse' | 'polygon';
  /**
   * `polygon` 일 때 꼭짓점. 상자 왼쪽 위를 (0,0) 으로 한 **pt** 자리다.
   * 안 주면 이등변 삼각형. 세 점보다 적으면 거절한다.
   */

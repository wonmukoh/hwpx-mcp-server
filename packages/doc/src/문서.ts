...
export class 문서 {
  ...
  칸글바꾸기(id: string, 새글: string): 결과<{ 바뀐수: number; 잃은서식: number }> {
    ...
    // 문단과 같다 — 이미 같았으면 **0 이지 실패가 아니다** (위 설명 참고).
  ...
  글바꾸기(id: string, 새글: string): 결과<{ 바뀐수: number; 잃은서식: number }> {
    ...
    // **「이미 같았다」는 실패가 아니라 0 이다.**
    //
    // `도구.ts` 에 이렇게 적어 뒀다 — 「0 은 찾았는데 이미 그랬다, 실패는 못
    // 찾았거나 못 한다. 둘을 섞으면 조용한 실패가 된다.」 그런데 여기는 0 을
    // 실패로 냈다. 제 원칙을 제가 어긴 것이다.
    //
    // 값이 비쌌다. 양식을 채울 때는 **이미 맞게 적힌 칸이 섞이기 마련**인데
    // (머리글·고정 문구), 그것 하나가 `edit` 묶음을 거기서 멈춰 세운다.
    // 실제로 고침 81개짜리 묶음이 31번째에서 죽어 뒤 50개가 안 들어갔다.
    // 부르는 쪽은 「이미 그랬다」와 「못 했다」를 가려야 하는데, 그건
    // `changed: 0` 으로 충분히 보인다.

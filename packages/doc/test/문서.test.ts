 ...
 * 3단계 합격 기준:
 *
 * - 문단 넣기·글 바꾸기·서식 주기가 **ID 로** 되고, **연 문서 안에서는 삽입해도
 *   ID 가 안 밀린다**. 저장하고 다시 열면 이야기가 다르다 — `식별자.test.ts` 참고.
...
describe('글 바꾸기', () => {
  ...
  it('**같은 글을 주면 0 이라고 한다 — 실패가 아니다**', () => {
    // 전에는 실패로 냈다. 값이 비쌌다 — 양식을 채울 때는 **이미 맞게 적힌
    // 칸이 섞이기 마련**인데(머리글·고정 문구), 그것 하나가 `edit` 묶음을
    // 거기서 멈춰 세웠다. 고침 81개짜리 묶음이 31번째에서 죽어 뒤 50개가
    // 안 들어간 일이 있었다.
    //
    // 「이미 그랬다」와 「못 했다」는 갈라야 하지만, 그건 `바뀐수: 0` 으로
    // 충분히 보인다. 조용한 것이 아니다 — **말은 하되 멈추지는 않는다.**
    const { d } = 열기();
    d.ID매기기();
    const p = d.구역들[0]!.문단들.find((x) => !x.비었나)!;
    const id = d.이름표.아이디(p.el);
    ...
    if (!r.ok) return;
    expect(r.value.바뀐수, '아무것도 안 바뀌었다고 말해야 한다').toBe(0);
  });

  it('**이미 같은 칸이 섞여도 묶음이 안 멈춘다**', () => {
    // 위와 같은 까닭. 칸도 문단과 같이 굴어야 한다 — 전에는 칸만 실패로 냈다.
    const { d } = 열기('ref-table-basic.hwpx');
    d.ID매기기();
    // **건너뛰지 않는다.** 기준 파일에 표가 없으면 그건 기준 파일이 깨진 것이지
    // 이 시험이 지나갈 일이 아니다 — 건너뛰면 그 갈래에서 아무것도 안 본다.
    const 표 = d.구역들[0]!.표들[0];
    expect(표, 'ref-table-basic 에 표가 있어야 이 시험이 뭔가를 본다').toBeDefined();
    const 칸아이디 = d.셀아이디(d.이름표.아이디(표!), 0, 0);
    const 지금 = d.찾기(칸아이디);
    expect(지금.ok && 지금.value.갈래 === '셀', '0,0 칸을 찾을 수 있어야 한다').toBe(true);

    const 먼저 = d.칸글바꾸기(칸아이디, '새 값');
    expect(먼저.ok).toBe(true);
    const 다시 = d.칸글바꾸기(칸아이디, '새 값');
    expect(다시.ok, '같은 값을 또 줘도 실패가 아니다').toBe(true);
    if (!다시.ok) return;
    expect(다시.value.바뀐수).toBe(0);
  });

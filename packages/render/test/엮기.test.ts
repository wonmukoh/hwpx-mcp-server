...
describe('바탕쪽 — 못 그리는 것을 말한다', () => {
  it('**바탕쪽이 있으면 그렇다고 알린다**', () => {
    // 모든 쪽 뒤에 깔리는 것이라 쪽마다 겹쳐 그려야 맞는데, 우리는 쪽을 나눠
    // 그리지 못한다. 못 그리는 것은 죄가 아니다 — **조용한 것이 죄다.**
    const 없는것 = 엮은('ref-text-basic.hwpx');
    expect(없는것.못옮긴것, '바탕쪽이 없는 문서엔 안 붙어야 한다').not.toContain('바탕쪽');

    const 있는것 = 엮은('ref-masterpage.hwpx');
    expect(있는것.못옮긴것).toContain('바탕쪽');
  });
});

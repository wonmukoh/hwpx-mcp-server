...

  {
    ...
    outputSchema: 묶음('문서 뼈대', {
      ...
      items: 목록('요소들', 묶음('요소 하나', {
        ...
        // 글상자 안 글은 **쪽에 보이는 본문인데 문단 밑이 아니다.** 어디서 온
        // 것인지 밝혀 준다 — 안 밝히면 같은 글이 두 군데 있는 것처럼 보인다.
    ...
    처리: 검사하고<{ doc_id: string; section?: number; limit?: number; in_tables?: boolean }>(뼈대스키마, (인자, 방) => {
      ...
      for (const p of s.문단들) {
        ...
        // **글상자 안 글은 뼈대에 안 나오고 있었다.**
        //
        // 도형 안 글은 `hp:drawText > hp:subList > hp:p` 에 있다. 문단 밑이
        // 아니라서 위 줄이 집는 `p.글` 에 안 잡힌다 — 도형이 든 문단은
        // **빈 문단으로만 보인다.**
        //
        // 학교·정부 양식은 **절 제목을 글상자에 넣는다** (실측: 글상자 90개가
        // 8편에). 그래서 뼈대만 믿고 채우면 「추진 개요」 같은 큰 제목에 옛
        // 사업명이 그대로 남는다. 실제로 그럴 뻔했다.
        //
        // 셀과 달리 `in_tables` 뒤에 숨기지 않는다 — 글상자 글은 쪽에 그대로
        // 보이는 **본문**이지, 파고들어야 나오는 속살이 아니다. 대신 어디서
        // 온 것인지 `in_shape` 로 밝힌다.
        for (const dt of findAll(p.el, 'hp:drawText')) {
          for (const q of findAll(dt, 'hp:p')) {
            것들.push({
              id: d.이름표.아이디(q), kind: 'paragraph',

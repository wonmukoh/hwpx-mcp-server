 ...
 *   Preview/*                미리보기 (없어도 된다)
 */
import { createEntry, entryData, readZip, setEntryData, writeZip, 압축방식정하기, STORE, } from './zip.js';
export const 부품 = {
    mimetype: 'mimetype',
    version: 'version.xml',
...
export class HwpxContainer {
    ...
    static open(buf) {
        ...
        try {
            ...
            throw new HwpxError(`HWPX 파일을 열 수 없다: ${e.message}`, 'HWPX 는 zip 이다. 파일이 온전한지, .hwp(이진 형식)를 .hwpx 로 잘못 부른 건 아닌지 보라.');
        }
        const c = new HwpxContainer(zip);
        const 첫항목 = zip.entries[0];
        if (!첫항목 || 첫항목.name !== 부품.mimetype) {
            throw new HwpxError(`첫 항목이 mimetype 이 아니다 (${첫항목?.name ?? '항목 없음'})`, 'HWPX 는 mimetype 이 반드시 첫 항목이어야 한다.');
        }
        if (!zip.byName.has(부품.header)) {
            throw new HwpxError(`${부품.header} 가 없다`, 'HWPX 가 아니거나 파일이 깨졌다.');
        }
        return c;
    }
    // ── 부품 읽기 ──────────────────────────────────────────────────────────
    ...
    read(name) {
        ...
        if (!e) {
            throw new HwpxError(`${name} 부품이 없다`, `이 문서에 있는 부품: ${this.names().slice(0, 8).join(', ')}…`);
        }
        return entryData(e);
    }
    /** 부품을 글자로 (UTF-8) */
    ...
    save() {
        const 문제 = this.검사();
        if (문제.length > 0) {
            throw new HwpxError(`저장을 막았다. 이대로 쓰면 한글이 못 읽는다:\n${문제.map((s) => `  - ${s}`).join('\n')}`, '위 문제를 고치고 다시 저장하라.');
        }
    ...
    검사() {
        ...
        if (!this.zip.byName.has(부품.header))
            문제.push(`${부품.header} 가 없다`);
        if (this.sectionNames().length === 0)
            문제.push('구역(section) 파일이 하나도 없다');
...
export function 부품압축방식(c, name) {
    // 컨테이너 안쪽을 들여다보는 유일한 통로
    return c.zip.byName.get(name)?.method;
}

...
export function readZip(buf) {
    ...
    if (eocd === -1)
        throw new ZipError('zip 이 아니다 (중앙 디렉터리 끝을 못 찾음)');
    ...
    for (let i = 0; i < 항목수; i++) {
        ...
        const entryComment = Buffer.from(buf.subarray(p + 46 + nameLen + extraLen, p + 46 + nameLen + extraLen + commentLen));
        ...
        const extraLocal = Buffer.from(buf.subarray(localOff + 30 + lhNameLen, localOff + 30 + lhNameLen + lhExtraLen));
 ...
 */
export function 압축방식정하기(name) {
    const ext = (name.split('.').pop() ?? '').toLowerCase();
    // 이미 압축된 형식
    if (['jpg', 'jpeg', 'png', 'gif', 'zip', 'jp2', 'webp'].includes(ext))
        return STORE;
    return DEFLATE;
}
/** 항목 내용을 갈아 끼운다 */
export function setEntryData(e, data, method) {
...
/** 새 항목을 만든다 */
export function createEntry(name, data, method) {

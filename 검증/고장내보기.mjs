...
const 고장들 = [
  ...
  [
    '바탕쪽이 있는데 엮을 때 아무 말도 안 한다 (있던 글이 없어진 줄도 모른다)',
    'packages/render/src/엮기.ts',
    "  if (d.컨테이너.바탕쪽이름들().length > 0) c.못옮긴것.add('바탕쪽');",
    '  void d;',
    'packages/render',
  ],
  [
    '구역 둘이 바탕쪽 부품을 나눠 쓰게 한다 (**한글이 죽는다** — 못 여는 것보다 나쁘다)',
    'packages/doc/src/문서.ts',
    '      const 낸것 = this.통.바탕쪽더하기(xml, s.이름);',
    "      const 낸것 = 첫id === undefined ? this.통.바탕쪽더하기(xml, s.이름) "
      + ": { 이름: '', id: 첫id };",
    'packages/doc',
  ],
  [
    '바탕쪽 부품을 늘 첫 구역 앞에 둔다 (한글이 놓는 차례와 어긋난다)',
    'packages/doc/src/문서.ts',
    '      const 낸것 = this.통.바탕쪽더하기(xml, s.이름);',
    '      const 낸것 = this.통.바탕쪽더하기(xml, this.구역이름들[0] ?? s.이름);',
    'packages/doc',
  ],
  [
    '바탕쪽을 내고 masterPageCnt 를 안 세운다 (요소는 있는데 한글이 안 읽는다)',
    'packages/doc/src/본문.ts',
    "    setAttr(sp, 'masterPageCnt', String(childrenNamed(sp, 'hp:masterPage').length));",
    "    setAttr(sp, 'masterPageCnt', '0');",
    'packages/doc',
  ],
  [
    '바탕쪽 부품을 manifest 에 안 적는다 (한글이 부품을 못 찾는다)',
  ...
  [
    '바탕쪽을 spine 에도 넣는다 (한글이 본문 차례에 끼워 읽는다)',
    'packages/hwpx/src/container.ts',
    '      // **spine 에는 안 넣는다.** 넣으면 한글이 바탕쪽을 본문 차례에 끼워 읽는다.',
    '      hpf = hpf.replace(\'</opf:spine>\', `<opf:itemref idref="${id}" linear="yes"/></opf:spine>`);',
    'packages/doc',
  ],
  [
    '바탕쪽 글 자리를 0 으로 둔다 (한글이 넣는 값과 달라진다)',
    'packages/doc/src/문서.ts',
  ...
  [
    '바탕쪽에 쪽 설정이 든 런도 넣는다 (쪽 설정이 둘인 문서가 된다)',
    'packages/doc/src/본문.ts',
    "      if (아이들.some((c) => c.name !== 'hp:t')) { removeNode(r); continue; }",
    "      if (아이들.some((c) => c.name !== 'hp:t')) { continue; }",
    'packages/doc',
  ],
  [
    '뼈대가 글상자 안 글을 안 준다 (절 제목이 통째로 안 보인다)',
    'packages/server/src/도구.ts',
    "        for (const dt of findAll(p.el, 'hp:drawText')) {",
    '        for (const dt of []) {',
    'packages/server',
  ],
  [
    '뼈대가 글상자 글을 어디서 온 것인지 안 밝힌다 (본문과 뒤섞여 보인다)',
    'packages/server/src/도구.ts',
    "              preview: 미리보기(new 문단(q, s.source).글.trim()), in_shape: true,",
    "              preview: 미리보기(new 문단(q, s.source).글.trim()),",
    'packages/server',
  ],
  [
    '「이미 같은 글」을 실패로 되돌린다 (고침 묶음이 거기서 죽는다)',
    'packages/doc/src/문서.ts',
    "    return this.남기기('글바꾸기', id, p.value.글바꾸기(새글));",
    ["    const r0 = p.value.글바꾸기(새글);",
      "    if (r0.ok && r0.value.바뀐수 === 0) return this.남기기('글바꾸기', id, 안됨('없다', 'x'));",
      "    return this.남기기('글바꾸기', id, r0);"],
    'packages/doc',
  ],
  [
  ...
  [
    'colCnt 를 칸수로 넣는다 (칸수가 colCnt 를 읽어 제자리를 맴돈다)',
    'packages/doc/src/표.ts',
    "    setAttr(this.el, 'colCnt', String(넓이));",
    "    setAttr(this.el, 'colCnt', String(this.칸수));",
    'packages/doc',
  ],
  [
    '새 칸에 이웃 폭을 그대로 준다 (표가 넓어져 쪽 밖으로 나간다)',
    'packages/doc/src/표.ts',
    '      const 고르게 = Math.floor(총폭 / 새수);',
    '      const 고르게 = 옛폭[0] ?? Math.floor(총폭 / 새수);',
    'packages/doc',
  ],
  [
    '합친 칸을 가로지르는 자리에도 칸을 넣는다 (격자가 무너진다)',
    'packages/doc/src/표.ts',
  ...
  [
    '합침을 풀 때 colSpan 을 안 되돌린다 (푼 척만 한다)',
    'packages/doc/src/표.ts',
  ...
  [
    '칸 채우기를 hc:solidFill 에서 찾는다 (실제 문서에 없는 이름 — 배경이 통째로 빠진다)',
    'packages/render/src/서식읽기.ts',
    "  const 통 = firstChildNamed(borderFill, 'hc:fillBrush');",
    "  const 통 = firstChildNamed(borderFill, 'hc:solidFill');",
    'packages/render',
  ],
  [
    'faceColor=none 을 색으로 친다 (안 칠한 칸까지 칠해진다)',
    'packages/render/src/서식읽기.ts',
  ...
  [
    'manifest 항목을 목록 밖에 붙인다 (구역을 더할 때)',
    'packages/hwpx/src/container.ts',
    // 바탕쪽 부품도 같은 줄을 쓴다 — 앞 줄까지 겨눠 구역 쪽만 고른다
    ["      hpf = hpf.replace('</opf:manifest>', `${항목}</opf:manifest>`);",
      '',

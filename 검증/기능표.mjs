...
const 기능들 = [
  ...
  // **`hp:secPr` 안이 아니라 딴 부품이다.** 규격만 보고 안에 넣었다가 한글이
  // 파일을 아예 못 여는 것을 보고 물렀다 (자료/실측.md 32항).
  { 갈래: '문서·구역', 이름: '바탕쪽',
    블록: [{ kind: 'body', text: '가' }],
    고침: () => [{ op: 'set_master_page', text: '내부 검토용' }],
    본다: (d) => {
      // 부품·가리킴·개수 셋이 다 맞아야 한글이 읽는다. 하나만 봐도 안 되고,
      // 부품 없이 개수만 세우면 **문서가 아예 안 열린다.**
      const 부품들 = d.컨테이너.바탕쪽이름들();
      const sp = d.구역들[0]?.쪽설정;
      if (!부품들.length || !sp) return false;
      return 속성(sp, 'masterPageCnt') === '1'
        && d.구역들[0].바탕쪽참조들.length === 1
        && d.바탕쪽들.includes('내부 검토용');
    } },
  ...
  //
  // 실측: 글상자 90개(8편)로, 아직 없는 것들을 다 합친 것보다 많다.
  // 다만 붙는 자리가 실물과 다르다 — 실물 90개는 hp:shadow 와 hc:pt0 사이인데
  // 우리는 맨 뒤에 붙인다. 한글은 그래도 읽는다(재 봤다).
  { 갈래: '개체', 이름: '글상자(도형 안 글)',
    블록: [{ kind: 'shape', text: '상자 안 글', width: 300, height: 60 }],
    본다: (d) => {
      const 상자들 = findAll(d.구역들[0].root, 'hp:drawText');
      if (!상자들.length) return false;
      const 글 = 상자들.flatMap((e) => findAll(e, 'hp:t').map((t) => textOf(t))).join('');
      // **글이 상자 「안」에 있어야 한다.** 도형 뒤 문단에 있으면 상자와 같이 안 움직인다.

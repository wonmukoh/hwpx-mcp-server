...
$목록 = [ordered]@{
    ...
    "ref-masterpage" = @{
        확인 = "바탕쪽 — 딴 부품(Contents/masterpageN.xml)에 담기나"
        만들기 = {
            글 "본문 한 줄"
            # **HAction.Run 으로 부르면 대화상자에서 멈춘다.** 창을 숨겨 놨으니
            # 눈에도 안 보이고, 멈춘 한글이 남아 **그 뒤의 한글 시험이 전부
            # OPENFAIL 로 떨어진다** (2026-09-10 에 기준 파일 27편이 다 못 열렸다).
            #
            # 매개변수 집합으로 부르면 대화상자가 안 뜬다. 액션 이름이 같아도
            # **부르는 길에 따라 대화상자가 뜨고 안 뜬다** — 다른 대화상자 액션도
            # 마찬가지일 것이다.
            $h = $script:hwp
            $act = $h.CreateAction("MasterPage"); $set = $act.CreateSet(); $act.GetDefault($set)
            try { $set.SetItem("Type", 0) } catch { }   # 0 = 양쪽
            $act.Execute($set) | Out-Null
            글 "바탕쪽 글"
            실행중하나 @("CloseEx","Close") | Out-Null
        }

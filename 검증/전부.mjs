...
//
// **사용자의 개인 문서 폴더는 안 본다.** 예전에는 `~/OneDrive/문서`(96편)와
// `~/OneDrive/바탕 화면`(43편)까지 훑었다. 열어 읽기만 하고 원본은 안 건드리지만,
// 학생·개인 정보가 든 문서가 섞여 있고 **파일 이름이 아래 목록 파일에 평문으로
// 남는다.** 재료가 더 필요하면 `자료/표본/로컬` 에 **골라서** 넣는 것이 맞다.
// (2026-09-07 사용자 결정. 184편 → 45편으로 준다.)
const 문서폴더 = [
...
const 할일 = [
  ...
  // **도구 표면을 통해 한 번도 안 걸리는 op 이 있나.** 문서 층 시험이 있어도
  // 그것은 문서 층만 지킨다 — 도구 이름·인자·ID 앞머리는 거기서 안 걸린다.
  ['고침 훑기 (op 이 도구로 걸리나)', process.execPath, [path.join('검증', '고침훑기.mjs')]],
...
if (한글까지) {
  ...
  // HTML 로 엮은 것이 원본과 닮았나 — 한글 PDF 와 브라우저 PDF 를 견준다.
  // **글이 사라지는 것만** 갈래를 깨뜨린다. 쪽수는 다를 수밖에 없다.
  할일.push(['HTML 로 엮기 (한글 PDF 와 견주기)', process.execPath, [path.join('검증', 'HTML닮음.mjs')]]);
}
...
// ── 끝나면 dist 를 다시 굽는다 ────────────────────────────────────────────
//
// **검증이 도는 동안 dist 가 딴 것이 될 수 있다.** 고장 내보기는 소스에 고장을
// 심고 vitest 를 부르는데, 그때 dist 를 굽는 시험이 섞여 있으면 고장이 담긴 채
// 구워진다. 소스는 되돌아가지만 **dist 는 안 되돌아간다.**
//
// `굽는동안.test.ts` 는 이제 제 임시 자리에 굽게 고쳤다. 그래도 여기서 한 번 더
// 굽는다 — 앞으로 어느 갈래가 또 dist 를 건드려도 **끝난 뒤의 dist 는 지금
// 소스에서 나온 것**이라야 한다. `dist/` 는 `.gitignore` 라 git 이 못 잡는다.
// 아래로 쓰는 앱이 저장소 dist 를 물고 번들을 굽는다 (`자료/실측.md` 28항).
console.log('── 끝에 dist 다시 굽기 ─────────────────────────────');
{
  const r = spawnSync(process.execPath, [path.join('검증', '빌드.mjs'), '--배포'],
    { stdio: 'inherit' });
  결과.push(['끝에 dist 다시 굽기', r.status === 0]);
}
console.log();
